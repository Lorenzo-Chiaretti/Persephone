<template>
  ...
  >
    ...
    <div ref="overlayRef" class="absolute inset-0 pointer-events-none">
      ...
      >
        <!-- Top: listening indicator + chat bubble -->
        ...
        <!-- Debug buttons -->
        <div class="absolute left-4 top-1/4 flex flex-col gap-2 pointer-events-auto">
          <p class="text-[10px] text-white bg-black/40 p-1 rounded uppercase font-bold">
            Debug Loc:
          </p>
          <button
            @click="testPoi('via-senato')"
            :class="arStore.selectedPoi?.id === 'via-senato' ? 'bg-green-600 scale-105' : 'bg-orange-500'"
          ...
          <button
            ...
            :class="arStore.selectedPoi?.id === 'laghetto-marco' ? 'bg-green-600 scale-105' : 'bg-orange-500'"
            class="text-white p-2 text-xs rounded shadow-lg transition-all"
          >
            📍 Laghetto
          </button>
        ...
        <!-- Bottom: chat input + voice/exit buttons -->
      ...
      >
        <div class="bg-black/60 text-white px-6 py-3 rounded-full backdrop-blur flex items-center gap-2">
      ...
      >
        <svg width="40" height="40" viewBox="0 0 40 40">
          <line x1="20" y1="10" x2="20" y2="30" stroke="white" stroke-width="2" stroke-linecap="round" />
