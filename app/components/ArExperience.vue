<template>
  <div class="fixed inset-0 z-50 pointer-events-none">
    ...
    <div
      v-if="modelPlaced && !waterVisible"
      class="absolute inset-x-4 top-4 z-20 flex flex-col items-center gap-3"
    >
      ...
      <Transition name="fade-slide">
        ...
        >
          ...
          <div
            class="absolute -top-10 -left-10 w-24 h-24 bg-cyan-500/20 rounded-full blur-2xl animate-pulse pointer-events-none"
          />
          <div
            class="absolute -bottom-10 -right-10 w-24 h-24 bg-blue-600/20 rounded-full blur-2xl animate-pulse pointer-events-none"
          />

          <!-- Mic Icon and Pulse Animation -->
          <div
            class="relative w-12 h-12 flex items-center justify-center rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400"
          >
            <!-- Pulsing outer rings -->
            <div
              class="absolute inset-0 rounded-full bg-cyan-400/20 animate-ping opacity-60 scale-125"
            />
            <div
              class="absolute -inset-1 rounded-full border border-cyan-400/30 animate-pulse"
            />
          ...
          <div class="text-center z-10">
            <p
              class="text-[11px] uppercase tracking-wider text-cyan-400 font-semibold mb-0.5"
            >
              {{
            ...
            <p class="text-white text-[15px] font-medium leading-tight">
              {{
                locale?.toLowerCase()?.startsWith('it')
                  ? 'Pronuncia "Risveglia i navigli!"'
                  : 'Say "Bring the water back!"'
              }}
            </p>
          </div>
          ...
          <div class="flex items-center gap-1 h-3 my-1">
            <span
              class="w-[3px] h-2 bg-cyan-400 rounded-full animate-wave-bar"
              style="animation-delay: 0s"
            />
            <span
              class="w-[3px] h-3 bg-cyan-400 rounded-full animate-wave-bar"
              style="animation-delay: 0.15s"
            />
            <span
              class="w-[3px] h-1.5 bg-cyan-400 rounded-full animate-wave-bar"
              style="animation-delay: 0.3s"
            />
            <span
              class="w-[3px] h-4 bg-cyan-400 rounded-full animate-wave-bar"
              style="animation-delay: 0.45s"
            />
            <span
              class="w-[3px] h-2.5 bg-cyan-400 rounded-full animate-wave-bar"
          ...
          <div
            class="w-full h-1 bg-white/10 rounded-full overflow-hidden absolute bottom-0 left-0 right-0"
          >
            <div
...
const startVoiceListening = async () => {
  ...
  if (wasNonnaListening) {
    console.log(
      '👵 Nonna sta ascoltando. Spegnimento temporaneo per evitare conflitti microfono...'
    )
    stopNonnaAll()
  ...
  timerInterval = setInterval(() => {
    ...
    if (elapsed >= absoluteLimit) {
      console.log(
        '⏳ Timeout assoluto (30s) raggiunto. Disattivazione totale del microfono e Deepgram per risparmio token.'
      )
      stopVoiceListening()
    }
  }, intervalTime)
  ...
  try {
    if (typeof navigator !== 'undefined' && 'audioSession' in navigator) {
      try {
        ...
        console.log(
          '📱 AudioSession impostata su play-and-record per il microfono (Water).'
        )
      } catch (err) {
    ...
    try {
      ...
      console.warn(
        '❌ First getUserMedia failed, trying with simple audio: true',
        err
      )
    ...
    socket.onmessage = (message) => {
      ...
      if (transcript && transcript.length > 0) {
        ...
        if (matchItalian || matchEnglish) {
          console.log(
            '✅ Comando vocale rilevato (unificato)! Attivazione acqua in corso...',
            lowerTranscript
          )
          triggerWater()
        }
